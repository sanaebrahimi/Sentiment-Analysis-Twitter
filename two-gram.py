...
def sentiment_word(pos_data, neg_data):
    ...
    for i in range(2):
        for index, tweets in data[i].iterrows():
            # for index, row in tweets.iterrows():
            text = tweets['Anootated tweet']
            wordcount[i] += process_dict(text, dictlist, i)
...
def process_2gram(pos_data, neg_data):
    global wordpat
    global sentiment_words
    ...
    for i in range(2):
        for index, tweets in data[i].iterrows():
            l = []
            # for index, row in tweets.iterrows():
            for w in wordpat.finditer(tweets['Anootated tweet']):
                text = tweets['Anootated tweet']
                wordcount[i] += 1
                x, y = w.span()
                            # print(w)
                k = wordnet_lemmatizer.lemmatize(text[x:y].lower(), pos="v")
...
def process(s):
    ...
    for key in list_of_subsets:
        ...
        if (key[0], key[1]) in twogram_words:
            ...
            pos_bifactors = twogram_words[(key[0], key[1])][0] / (
...
if __name__ == "__main__":
    ...
    positive_data = X_train.loc[X_train['Class'] == 1]
    negative_data = X_train.loc[X_train['Class'] == -1]
    neutral_data = X_train.loc[X_train['Class'] == 0]

    sentiment_word(positive_data, negative_data)
    process_2gram(positive_data, negative_data)
    ...
    for index, row in X_test.iterrows():
        ...
        p_score, n_score, xy, slist = process(text)
